...
@protocol AWKAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(AWKAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface AWKAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<AWKAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation AWKAnimatedImageView
...
{
    // Presision is set to half of the `kAWKAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kAWKAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            AWKWeakProxy *weakProxy = [AWKWeakProxy weakProxyForObject:self];
