...
class ProfileViewController: BeamViewController {
    ...
    internal func karmaStringFromValue(_ value: Int?) -> NSAttributedString {
        ...
        if numberString.characters.count < 7 {
            for _ in 1...(7-numberString.characters.count) {
                paddingString += "0"
            }
        }
        
        
        let numberAttributedString = NSMutableAttributedString(string: numberString, attributes: [NSForegroundColorAttributeName: UIColor.white])
        let paddingAttributedString = NSMutableAttributedString(string: paddingString, attributes: [NSForegroundColorAttributeName: UIColor.white.withAlphaComponent(0.3)])
...
// MARK: - EmbeddingLayoutSupport
extension ProfileViewController: EmbeddingLayoutSupport {
    
    func embeddedLayout() -> UIEdgeInsets {
        return UIEdgeInsets(top: 100 + self.toolbar.bounds.height, left: 0, bottom: self.contentInset.bottom, right: 0)
    }
