...
//  Based on FLAnimatedImage: https://github.com/Flipboard/FLAnimatedImage
//
...
const NSTimeInterval kAWKAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `AWKAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, AWKAnimatedImageDataSizeCategory) {
    AWKAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    AWKAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    AWKAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    AWKAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, AWKAnimatedImageFrameCacheSize) {
    AWKAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    AWKAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    AWKAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    AWKAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};


#if defined(DEBUG) && DEBUG
@protocol AWKAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(AWKAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(AWKAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(AWKAnimatedImage *)animatedImage;
@end
#endif


@interface AWKAnimatedImage ()
...
// The actual type of the object is `AWKWeakProxy`.
@property (nonatomic, strong, readonly) AWKAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<AWKAnimatedImageDebugDelegate> debug_delegate;
#endif
...
@implementation AWKAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > AWKAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > AWKAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [AWKAnimatedImage class]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            ...
            // Note that AWKAnimatedImages can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
...
- (instancetype)init
{
    AWKAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        FLLog(FLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `AWKAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled
{
    ...
    if (self) {
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        // Support frame delays as low as `kAWKAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
                        ...
                        if ([delayTime floatValue] < ((float)kAWKAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            FLLog(FLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kAWKAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
                    ...
                    skippedFrameCount++;
                    FLLog(FLLogLevelInfo, @"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, self.imageSource);
                }
            }
        }
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= AWKAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= AWKAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = AWKAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = AWKAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
        ...
        _weakProxy = (id)[AWKWeakProxy weakProxyForObject:self];
...
{
    AWKAnimatedImage *animatedImage = [[AWKAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    AWKAnimatedImage * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    } else if ([image isKindOfClass:[AWKAnimatedImage class]]) {
        AWKAnimatedImage *animatedImage = (AWKAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        FLLog(FLLogLevelError, @"`image` isn't of expected types `UIImage` or `AWKAnimatedImage`: %@", image);
    }
...
{
    self.frameCacheSizeMaxInternal = AWKAnimatedImageFrameCacheSizeNoLimit;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(AWKAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    FLLog(FLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)AWKAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = AWKAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(AWKAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
@implementation AWKAnimatedImage (Logging)
...
#pragma mark - AWKWeakProxy

@interface AWKWeakProxy ()
...
@implementation AWKWeakProxy

#pragma mark Life Cycle

// This is the designated creation method of an `AWKWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject
{
    AWKWeakProxy *weakProxy = [AWKWeakProxy alloc];
