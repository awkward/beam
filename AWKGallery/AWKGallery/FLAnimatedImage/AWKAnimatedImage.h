...
extern const NSTimeInterval kAWKAnimatedImageDelayTimeIntervalMinimum;

//
//  An `AWKAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `AWKAnimatedImageView`.
...
//
@interface AWKAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `AWKAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `AWKAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface AWKAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [AWKAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface AWKWeakProxy : NSProxy
